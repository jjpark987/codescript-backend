...
async def post_problem(db: Session, problem: ProblemCreate) -> ProblemResponse:
    ...
    if not subcategory: 
        raise HTTPException(status_code=404, detail=f'❌ CRUD subcategory id {problem.subcategory_id} not found.')
...
async def get_problem(db: Session, problem_id: int) -> ProblemResponse:
    ...
    if not problem:
        raise HTTPException(status_code=404, detail=f'❌ CRUD problem_id = {problem_id} not found')
